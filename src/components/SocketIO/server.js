...
class SocketServer {
    ...
    #onConnection = (socket) => {
        try {
            ...
            //TODO: ME FALTA CARGAR LOS DATOS DESPUES DE UNA DESCONEXION PARA QUE CARGUE TODO EL CHAT CUANDO SE VUELVA A CONECTAR YA TENGA TODOS SUS MENSAJES CARGADOS
        ...
        try {
            ...
            if (this.socketMap.get(user)) {
                if (typeFile === 'image') {
                    const modified = await this.dbMessage.insertTo({
                        option: 'image',
                        params: [file, user, this.socketMap.get(user), 'direct', message, date]
                    })
                    if (!modified) return false

                    this.io.to(this.socketMap[socket]).emit('direct_message', { file, typeFile, message })
                    return true;
                }
                return false;
                ...
                if (typeFile === 'image') {
                    ...
                    this.io.to(destination).emit('broadcast_message', { file, typeFile, message });
                    return true;
                }
                ...
                if (typeFile === 'image') {
                    ...
                    this.io.to(destination).emit('room_message', { file, typeFile, message })
                    return true;
                }
            }
    ...
    #eventBroadcast = async (data) => {
        ...
        console.log(`Envias por el nameSpace ${namespace} el mensaje de ${message}, el dia de ${date}`);
